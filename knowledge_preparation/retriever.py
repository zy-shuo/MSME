...
def search_target(target: str, api_key: Optional[str] = None, num_results: int = 3) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if "organic_results" not in results or len(results["organic_results"]) == 0:
            logger.warning(f"No organic results found for target: {target}")
            return []
...
def process_dataset_targets(dataset_path: str, output_path: Optional[str] = None) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    try:
        ...
        for target in targets:
            ...
            search_results = search_target(target_str)
